...
uses
  ...
  cxProgressBar,
  dxSkinsCore,
  dxSkinsDefaultPainters, uADPhysMSSQL,u_wait;
type
  TForm1 = class(TForm)
    OpenDlg: TOpenDialog;
    ...
    SP_AFTER
      : TADStoredProc;
    SP_PARAMS2_CLOSE: TADStoredProc;
    AdvEdit12: TAdvEdit;
    fcLabel1: TfcLabel;
    cxImage1: TcxImage;
    ...
    DatabaseODBC: TADConnection;
    wq_Info_Serveur: TADQuery;
...
begin
  ...
  // Recherche de la première fenêtre enfant direct
  while (Handle <> 0)
    do
  // Tant qu'on en trouve ....
    ...
    // libération de la chaîne
    StrDispose(p);
    RechercheEnfants(Handle);
    // Recherche de ses propres enfants
    Handle
      := GetNextWindow(Handle, GW_HWNDNEXT);
...
begin
    ...
    := StrAlloc(256);
  GetWindowText(Handle, p, 256);
  // Texte (éventuel) de la fenêtre
  s
    := inttostr(Handle) + ' - ' + p;
  Nom_Fenetre := PChar(p);
  if (copy
      (Nom_Fenetre, 1, 9) = 'GENESYS -') then
    begin
      RechercheEnfants(Handle);
    // Recherche des fenetres enfants
  end;
  StrDispose(p);
...
procedure
  TForm1.Fenetre_Position(Fenetre: TForm);
var
    Hauteur: Integer;
  Largeur: Integer;
  Top: Integer;
...
begin
    Ico := Ticon.Create;
  for i := IconesListe.Count + 1 to 150
    do
    begin
      IconesListe_Plus.GetIcon(0, Ico);
    IconesListe.AddIcon(Ico);
  end;
  for i := IconeListePNG.Count + 1 to 150
    do
    begin
      IconesListe_Plus.GetIcon(0, Ico);
    IconeListePNG.AddIcon(Ico);
  end;
  for i := 0 to IconesListe_Plus.Count - 1
    do
    begin
...
Function TForm1.GetID_Session(): String;
Var
    Tmp_ID_Session: String;
begin
    wq_Session.open;
  Tmp_ID_Session := wq_Session.FieldByName('ID').AsAnsiString;
  wq_Session.close;
  GetID_Session := Tmp_ID_Session;
end;
...
begin
  ...
  try
    ...
    ThreadAnimate
      := TThreadAnimate.Create(Form1,Edit3, AdvCircularProgress1);
    ThreadAnimate.Priority := tpHighest;
    ThreadAnimate.Resume;
    ...
    // ****************
    // La connexion en MSSQL
    // ****************
    wq_Info_Serveur.Open();
    Database.Params.Clear;
    Database.Params.Values['DriverID'] := 'MSSQL';
    Login := Database.Params.Values['User_Name'];
    Password := Database.Params.Values['Password'];
    Serveur := wq_Info_Serveur.FieldByName('Serveur').AsString;
    Base := wq_Info_Serveur.FieldByName('Base').AsString;

    Database.Params.Values['User_Name'] := DatabaseOdbc.Params.Values['User_Name'];
    Database.Params.Values['Password'] := DatabaseOdbc.Params.Values['Password'];

    Database.Params.Values['Server'] := wq_Info_Serveur.FieldByName('Serveur').AsString;
    Database.Params.Values['Database'] := wq_Info_Serveur.FieldByName('Base').AsString;
    Database.Params.Values['OSAuthent'] := 'Yes';
    try
      Database.Connected := True;
    except
      Database.Params.Values['OSAuthent'] := 'No';
      try
        Database.Connected := True;
      except
        showmessage(Database.Params.Text);
...
begin
  for i := 0 to ComponentCount - 1
    do
    if
      (Components[i] is TDataSet) and ((Components[i] as TDataSet).Active = True) then (Components[i] as TDataSet)
      .close;
  wq_Session.close;
  Database.close;
  ...
  if ThreadAnimate <> nil Then
    begin
      ThreadAnimate.Terminate;
    ThreadAnimate := nil;
  end;
end;
...
function
  TForm1.Procedure_Exist(Nom_procedure: string): Boolean;
begin
    Result
    := wq_Exist
    .locate('Nom', Nom_procedure, [loCaseInsensitive]);
end;
function TForm1.Param_Exist(Nom_procedure: string; Nom_Parametre: string): Boolean;
begin
    Result
    := wq_Exist_Param.locate('Recherche', Nom_procedure + '-' + Nom_Parametre, [loCaseInsensitive]);
end;
...
var
  ...
  int_Line
    , int_col, int_line_End, int_col_end, Col_Cpt_vide, Plag_Cpt_vide, Cpt_vide: Integer;
...
begin
  ...
  try
    ...
    choix := 0;
    SP_BEFORE
      .StoredProcName := Nom_procedure + '_BEFORE';
    With SP_BEFORE
      do
      begin
        Param := Params.Add;
    ...
    if (Procedure_Exist(Nom_procedure + '_PARAMS2') = True) then
      begin
        SP_PARAM.StoredProcName := Nom_procedure + '_PARAMS2';
    end
    else
      begin
    ...
    if SP_PARAM.StoredProcName = Nom_procedure + '_PARAMS2' then
      begin
      try
        Param := SP_PARAM.Params.Add;
      ...
      // Gestion du message PARAMS2_CLOSE
      SP_PARAMS2_CLOSE.StoredProcName := Nom_procedure + '_PARAMS2_CLOSE';
      With SP_PARAMS2_CLOSE
        do
        begin
          Params.Clear;
        Param := Params.Add;
      ...
      if SP_PARAM.Active = True then
        ...
        try
          try
            // Enleve le topmost
            SetWindowPos(HWND(Form1.Handle),
              // handle of window
              HWND_NOTOPMOST or WS_EX_APPWINDOW,
            ...
            F_Params2 := TF_Params2.Create(Self);
            HandleGeneMail := F_Params2.Handle;
            F_Params2.wsp_Filtres := SP_PARAM;
            F_Params2.ApplicationHandle := Form1.Handle;
            F_Params2.Timer := 0;
            if SP_PARAM.FindField('Timer') <> nil then
              F_Params2.Timer := SP_PARAM.FieldByName('Timer').asinteger;
            F_Params2.Init;
            ShowWindow(Form1.Handle, SW_HIDE);
            if F_Params2.ShowModal = mrok then
              begin
              for i := 0 to 9
                do
                begin
                  Tab_Params[i] := F_Params2.Tab_Params[i];
              end;
              F_Params2.Free;
              SP_PARAM.close;
              HandleGeneMail := 0;
              try
                if (Procedure_Exist(Nom_procedure + '_PARAMS2_CLOSE') = True) then
                  begin
                  for j := 0 to 8
                    do
                    begin
                      SP_PARAMS2_CLOSE.ParamByName('@Param' + inttostr(j)).Value := Tab_Params[j];
                  end;
                  SP_PARAMS2_CLOSE.ParamByName('@ID_Session_Temp').Value := F_Params2.ID_Session_Temp;
                  SP_PARAMS2_CLOSE.ExecProc;
                end;
              except
                Application.HandleException(Self);
                SP_PARAMS2_CLOSE.close;
                Fin_Appli;
                Exit;
              end;
            end
            ...
            SetWindowPos(HWND(Form1.Handle),
              // handle of window
              HWND_TOPMOST or WS_EX_APPWINDOW,
              // placement-order handle
              0,
              // horizontal position
              0,
    ...
    try
      if (Procedure_Exist(Nom_procedure + '_SELECT_FILE') = True) then
        begin
          SP_FICHIER.open;
        if
          (SP_FICHIER.FieldByName('Title').AsAnsiString <> '') then
          begin
    ...
    if (Procedure_Exist(Nom_procedure + '_LISTE_FICHIERS') = True) then
      ...
      while not SP_LISTE_FICHIERS.Eof
        do
        begin
        ...
        try
          ...
          while (not SP_COPY_FILE.Eof)
            ...
            if SP_COPY_FILE.FieldByName('Fonction').AsAnsiString <> '' then
              ...
              SP_COPY_FILE_AFTER.Params.ParamByName('@Fonction').AsAnsiString := SP_COPY_FILE.FieldByName('Fonction').AsAnsiString;
              // ['{E2F63D47-F74C-47B5-B90D-A5C009BA00FB}']
              ...
              if
                (Exist_Copy_After = True) or (Procedure_Exist(Nom_procedure + '_COPY_FILE_AFTER') = True) then
                begin
    ...
    if (Procedure_Exist(Nom_procedure + '_READ_XLS') = True) then
      ...
      if SP_EXCEL_READ_START.FieldByName('Cle_Session').AsAnsiString <> '' then
        ...
        if (Procedure_Exist(Nom_procedure + '_READ_XLS_REP') = True) then
          ...
          while not SP_EXCEL_READ_REP.Eof
            do
            begin
        ...
        if SP_EXCEL_READ.FieldByName('Fichier').AsAnsiString <> '' then
          ...
          while not SP_EXCEL_READ.Eof
            ...
            if (Fichier_excel <> Fichier_excel_old) or (Read_Excel_File_Ok = False) then
              begin
              if
                (Fichier_excel_old <> '') and (Read_Excel_File_Ok = True) then
                begin
                  Olexls.ActiveWorkbook.close(0);
              end;
            ...
            if SP_EXCEL_READ.FieldByName('Mode').AsAnsiString = 'TAB' then
              ...
              Cpt_vide := 0;
              for j := int_Line to int_line_End
                do
                begin
        ...
        if (Procedure_Exist(Nom_procedure + '_READ_XLS_MOVE') = True) then
          ...
          while not SP_EXCEL_READ_MOVE.Eof
            do
            begin
      ...
      if (Procedure_Exist(Nom_procedure + '_EXCEL') = True) then
        ...
        if SP_EXCEL.FieldByName('Fichier').AsAnsiString <> '' then
          ...
          while not SP_EXCEL.Eof
            ...
            if
              SP_EXCEL.FindField('Range_Columns_AutoFit') <> nil then
              begin
            ...
            if
              SP_EXCEL.FindField('Range_Columns_UnGroup') <> nil then
              begin
    ...
    try
      if
        (Procedure_Exist(Nom_procedure + '_OUTLOOK') = True) then
        begin
          SP_OUTLOOK_BEFORE.StoredProcName := Nom_procedure + '_OUTLOOK_BEFORE';
        With SP_OUTLOOK_BEFORE
          do
          begin
            Params.Clear;
          Param := Params.Add;
        ...
        try
          if (Procedure_Exist(Nom_procedure + '_OUTLOOK_AFTER') = True) then
            ...
            if
              (SP_OUTLOOK_AFTER.FieldByName('Msg').Asstring <> '') then
              begin
            ...
            if
              (SP_OUTLOOK_AFTER.FieldByName('Result').asinteger <> 1) then
              begin
                SP_OUTLOOK_AFTER.close;
              Fin_Appli;
              Exit;
    ...
    try
      if (Procedure_Exist(Nom_procedure + '_PRINT') = True) then
        ...
        if (Form1.Handle_ouverture > 0) then
          begin
            postmessage(HWND(Form1.Handle_ouverture), 2101, 0, 0);
    ...
    try
      if (Procedure_Exist(Nom_procedure + '_PRINT_MAIL') = True) then
        ...
        With SP_PRINT_BEFORE
          ...
          if Param_Exist
            (SP_PRINT_BEFORE.StoredProcName, '@Choix_Before_Principal') = True then
            begin
              Param := Params.Add;
            Param.Name
              := '@Choix_Before_Principal';
            Param.DataType := ftInteger;
            Param.ParamType := ptInput;
            Param.Value := Choix_Entree;
          end;
    ...
    SP_MEMOS
      .StoredProcName := Nom_procedure + '_MEMOS';
    With SP_MEMOS
      do
      begin
        Params.Clear;
    ...
    if
      (Procedure_Exist(Nom_procedure + '_MEMOS') = True) then
      begin
      try
        AdvCircularProgress1.Repaint;
        Application.ProcessMessages;
        ThreadSP := TThreadGenesysSP.Create(Edit3, SP, SP_MEMOS, Edit1, Edit2, wq_memos);
      ...
      try
        AdvCircularProgress1.Repaint;
        Application.ProcessMessages;
        ThreadSP := TThreadGenesysSP.Create(Edit3, SP, nil, Edit1, Edit2, wq_memos);
        ThreadSP.Priority := tpNormal;
        ThreadSP.Resume;
      except
        Application.HandleException(Self);
...
begin
  try
    try
      ...
      SP_TXT
        .StoredProcName := Nom_procedure + '_TXT';
      With SP_TXT
        do
        begin
          Params.Clear;
      ...
      try
        ...
        if (Procedure_Exist(Nom_procedure + '_TXT') = True) then
          ...
          for i := StringList.Count - 1 downto 0
            do
            begin
              StringList.Delete(i);
          end;
      ...
      // Lance la navigation
      SP_NAV
        .StoredProcName := Nom_procedure + '_NAV';
      With SP_NAV
        do
        begin
          Params.Clear;
        ...
        if (Procedure_Exist(Nom_procedure + '_CLOSE') = True) then
          ...
          while (not SP_CLOSE.Eof)
            ...
            if HAndle_close <> 0 then
              begin
                SendMessage(HWND(HAndle_close), WM_CLOSE, 0, 0);
              // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
            end
            else
              begin
              ...
              EnumWindows(@ListeChild, 0);
              // énumération des fenêtres Top-level
              if (Handle_Fenetre > 0) then
                begin
                  SendMessage(HWND(Handle_Fenetre), WM_CLOSE, 0, 0);
                // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
              end;
            end;
            SP_CLOSE.Next;
      ...
      SP_LOCK
        .StoredProcName := Nom_procedure + '_LOCK';
      With SP_LOCK
        do
        begin
          Params.Clear;
      ...
      try
        if (Procedure_Exist(Nom_procedure + '_LOCK') = True) then
          ...
          while (not SP_LOCK.Eof)
            ...
            if HAndle_close <> 0 then
              begin
                SendMessage(HWND(HAndle_close), 3100, Lock, 0);
              // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
              postmessage(HWND(HAndle_close), 3100, Lock, 0);
              // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
            end
            else
              begin
              ...
              if (Handle_Fenetre > 0) then
                begin
                  SendMessage(HWND(Handle_Fenetre), 3100, Lock, 0);
                // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
                postmessage(HWND(HAndle_close), 3100, Lock, 0);
                // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
              end;
      ...
      try
        if (N_Refresh > 0) then
          begin
          if N_Handle <> 0 then
            SendMessage(HWND(N_Handle), WM_G_RAFRAICHIR, 0, 0);
          // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
        end;
        // Navigation
        if (Nav_Fiche <> 0) then
          ...
          if N_Handle <> 0 then
            ...
            if Nav_Fiche > 0 then
              begin
                postmessage(HWND(N_Handle), 3152, 0, 0);
              // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
            end;
            if Nav_Fiche < 0 then
              postmessage(HWND(N_Handle), 3153, 0, 0);
            // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
          end;
        end;
          ...
          if Plusieurs_Fiches = 1 then
            ...
            try
              if (Procedure_Exist(Nom_procedure + '_LISTE_FICHES') = True) then
                ...
                try
                  if
                    (Procedure_Exist(Nom_procedure + '_LISTE_FICHES_BEFORE') = True) then
                    begin
    ...
    try
      if
        (Procedure_Exist(Nom_procedure + '_REFRESH') = True) then
        begin
          SP_REFRESH_BEFORE.StoredProcName := Nom_procedure + '_REFRESH_BEFORE';
        With SP_REFRESH_BEFORE
          do
          begin
            Params.Clear;
          Param := Params.Add;
        ...
        while (not SP_REFRESH.Eof)
          ...
          // énumération des fenêtres Top-level
          if
            (Handle_Fenetre > 0) then
            begin
              SendMessage(HWND(Handle_Fenetre), WM_G_RAFRAICHIR, 0, 0);
            // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
          end;
          SP_REFRESH.Next;
        end;
    ...
    // Gestion du message PARAMS_AFTER
    SP_AFTER
      .StoredProcName := Nom_procedure + '_AFTER';
    With SP_AFTER
      do
      begin
        Params.Clear;
...
begin
    fcLabel1.Caption := Edit3.Text;
  fcLabel1.Left := (Form1.Width - fcLabel1.Width) Div 2;
  if Edit3.Text = 'Fin de traitement ...' then
    begin
  ...
  if Edit3.Text = 'Un problème a été rencontré !' then
    begin
      AdvCircularProgress1.Visible := False;
  end;
end;
...
begin
  ...
  if
    SP_COPY_FILE.FieldByName('PossibleAnnuler').AsAnsiString = 'Oui' then
    begin
  ...
  if SP_COPY_FILE.FieldByName('ChangeNomSiCollision').AsAnsiString = 'Oui' then
    begin
      FlagsOptions
      := FlagsOptions + FOF_RENAMEONCOLLISION;
  end;
  if SP_COPY_FILE
    .FieldByName('SansConfirmation').AsAnsiString = 'Oui' then
    begin
      FlagsOptions := FlagsOptions + FOF_NOCONFIRMATION;
  end;
  if
    SP_COPY_FILE.FieldByName('SansProgression').AsAnsiString = 'Oui' then
    begin
  ...
  if SP_COPY_FILE.FindField('Fichiers_Seulement') <> nil then
    begin
    if SP_COPY_FILE
      .FieldByName('Fichiers_Seulement').AsAnsiString = 'Oui' then
      begin
  ...
  if SP_COPY_FILE
    .FindField('PasMessageErreur') <> nil then
    begin
  ...
  // On aurait aussi put ecrire pFrom := @Chaine[1]
  lpFileOp.fFlags := FlagsOptions;
  { fin de la préparation du paramètre lpFileOp }
  Resultat_copie := SHFileOperation(lpFileOp);
  // procède à la copie
...
var
    Rec: LongRec;
begin
    fc_version.Caption := GetAppVersionStr;
...
begin
  ...
  if (Form1.Handle_ouverture > 0) then
    ...
    wq_wm.close;
    postmessage
      (HWND(Form1.Handle_ouverture), WM_GENESYS, N_fiche, 0);
    // où Handle est le handle de la fenetre passé au MagicBtn par $HANDLE
  end;
...
var
  ...
  RetBuffer
    , FName, ResultBuffer: array [0 .. 255] of char;
  PIDL: PItemIDList;
begin
    StrPCopy(FName, Filename);
  // Initialize buffers
  FillChar
    (BrowseInfo, SizeOf(TBrowseInfo), #0);
...
begin
  ...
  if FindFirst(Path + Filename, Attribut, Rec) = 0 then
    try
      repeat
        if
          (Rec.Name <> '.') and (Rec.Name <> '..') and ((ListeInclude = 1) or (ListeInclude = 2)) then
          begin
        ...
        if
          (((Rec.Attr and faDirectory) > 0) Or (Rec.Name = '.') Or (Rec.Name = '..')) and ((ListeInclude = 2) or (ListeInclude = 3)) then
          begin
            wq_LISTE_FICHIERS.Append;
          wq_LISTE_FICHIERS.FieldByName('N_user').asinteger := N_user;
          wq_LISTE_FICHIERS.FieldByName('ID_Session').AsAnsiString := Cle_Session;
...
begin
    Result := 0;
  if (
    FileTime.dwLowDateTime = 0) and (FileTime.dwHighDateTime = 0) then
    Exit;
  try
